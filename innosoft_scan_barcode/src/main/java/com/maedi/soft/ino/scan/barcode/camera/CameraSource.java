...
public class CameraSource {
    ...
    public interface CommCameraSource
    {
        void backgroundLight(int isDark);
    }
    private CommCameraSource listener;
    ...
    public int mRequestedPreviewHeight = 1768;
    ...
    public static class Builder {
        ...
        //private CommCameraSource listenCamera;
        ...
        public Builder setCommCameraSource(CommCameraSource listen) {
            mCameraSource.listener = listen;
            return this;
        }
    ...
    public CameraSource start(SurfaceHolder surfaceHolder) throws IOException {
        synchronized (mCameraLock) {
            ...
            //Camera.Size optimalSize = getBestPreviewSize(mRequestedPreviewWidth,mRequestedPreviewHeight, params);
    ...
    public int doZoom(float scale) {
        synchronized (mCameraLock) {
            ...
            //Camera.Size size = getBestPreviewSize(mRequestedPreviewWidth,mRequestedPreviewHeight, parameters);
            //List<Camera.Size> supportedPreviewSizes = parameters.getSupportedPreviewSizes();
            //Camera.Size size = getOptimalPreviewSize(supportedPreviewSizes, mRequestedPreviewWidth, mRequestedPreviewHeight);
    ...
    private Camera createCamera() {
        ...
        //List<Camera.Size> supportedPreviewSizes = parameters.getSupportedPreviewSizes();
        //Camera.Size camSize = getOptimalPreviewSize(supportedPreviewSizes, mRequestedPreviewWidth, mRequestedPreviewHeight);
        ...
        //camera.setPreviewCallback(new CameraPreviewLightCallback());
    ...
    private byte[] createPreviewBuffer(Size previewSize) {
        ...
        Timber.d("START_BOX_SCAN CREATE_PREV - "+GraphicOverlay.startXPosScan +" | "+ GraphicOverlay.startYPosScan +" | "+ GraphicOverlay.widthPosScan +" | "+ GraphicOverlay.heightPosScan);
        Timber.d("START_BOX_SCAN CREATE_PREV - "+previewSize.getWidth() +" | "+ previewSize.getHeight());
        Timber.d("START_BOX_SCAN CREATE_PREV II - "+mRequestedPreviewWidth +" | "+ mRequestedPreviewHeight);
    ...
    private class CameraPreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            if(null != data) {
                Timber.d("START_BOX_SCAN NEXT-FRAME - "+mPreviewSize.getWidth() +" | "+ mPreviewSize.getHeight());
                //YuvImage yuvImage = new YuvImage(data, ImageFormat.NV21, mPreviewSize.getWidth(), mPreviewSize.getHeight(), null);
                //ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
                //yuvImage.compressToJpeg(new Rect(0, 0, mPreviewSize.getWidth(), mPreviewSize.getHeight()), 100, byteArrayOutputStream);
                ...
                //int bitsPerPixel = ImageFormat.getBitsPerPixel(ImageFormat.NV21);
                //int sizeInBits = bitmap.getHeight() * bitmap.getWidth() * bitsPerPixel;
                //int bufferSize = (int) Math.ceil(sizeInBits / 8.0d) + 1;
    ...
    private class CameraPreviewLightCallback implements Camera.PreviewCallback {
        @Override
        public void onPreviewFrame(byte[] data, Camera camera) {
        }
    }
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        void setNextFrame(byte[] data, Camera camera) {
            synchronized (mLock) {
                ...
                if (!mBytesToByteBuffer.containsKey(data)) {
                    Timber.d(TAG+"START_BOX_SCAN - Skipping frame.  Could not find ByteBuffer associated with the image " + "data from the camera.");
                    return;
                }
                ...
                Camera.Parameters parameters = camera.getParameters();
                int width = parameters.getPreviewSize().width;//GraphicOverlay.widthImageOri;//mPreviewSize.getWidth();
                int height = parameters.getPreviewSize().height;//GraphicOverlay.heightImageOri;//mPreviewSize.getHeight();
                ...
                //Bitmap bm = Bitmap.createBitmap(mPreviewSize.getWidth(), mPreviewSize.getHeight(), Bitmap.Config.RGB_565);
                //Log.d(TAG, "DATA_PREVIEW_CAMERA - "+bm);
                if(null != bm) {
                    //Log.d(TAG, "DATA_PREVIEW_CAMERA WIDTH - "+bm.getWidth());
                    if ((bm.getWidth() > 200) && (bm.getHeight() > 200)) {
                            ...
                            if (lum < 3) {
                                new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                                    @Override
                                    public void run() {
                                        //Toast.makeText(mContext, "DARK LIGHT", Toast.LENGTH_SHORT).show();
                                        Log.d(TAG, "DATA_PREVIEW_CAMERA - DARK LIGHT");
                                    }
                                }, 100);
                            }
                            else
                            {
                                new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                                    ...
                                    public void run() {
                                        //Toast.makeText(mContext, "DARK LIGHT", Toast.LENGTH_SHORT).show();
                                        Log.d(TAG, "DATA_PREVIEW_CAMERA - TOO LIGHT");
                                    }
                                }, 100);
                            }
                            */
                        //while (!haveProcessImage) {
                        haveProcessImage = true;
                        //if (timerValidation != null) {
                        //    timerValidation.cancel();
                        //}
                        ...
                        for (int j=0; j<pixels.length; j++) {
                            ...
                            //Log.d(TAG, "DATA_PREVIEW_CAMERA - CALC LUM -> "+luminance);
                            //if (lum < 0.5)
                            //{
                            //    darkPixels++;
                            //}
                            ...
                            int allPixelsCount = bm.getWidth() * bm.getHeight();
                        ...
                        if (darkPixels >= darkThreshold) {
                            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                                ...
                                public void run() {
                                    //Toast.makeText(mContext, "DARK LIGHT", Toast.LENGTH_SHORT).show();
                                    Log.d(TAG, "DATA_PREVIEW_CAMERA - DARK");
                        ...
                        //Log.d(TAG, "DATA_PREVIEW_CAMERA - COUNT LUM - "+lum);
                    ...
                    Timber.d(TAG+"START_RESIZE_IMAGE -> "+mPreviewSize.getWidth() +" | "+ mPreviewSize.getHeight());
                    Timber.d(TAG+"START_RESIZE_IMAGE -> "+bm.getWidth() +" | "+ bm.getHeight());
                    Timber.d(TAG+"START_RESIZE_IMAGE -> "+GraphicOverlay.startXPosScan+" | "+GraphicOverlay.startYPosScan +" | "+ GraphicOverlay.widthPosScan +" | "+ GraphicOverlay.heightPosScan);
                    //resizedbitmap1 = Bitmap.createBitmap(bm, GraphicOverlay.startXPosScan, GraphicOverlay.startYPosScan, 480, 640);
         ...
         */
        private boolean haveProcessImage;
        ...
        public void run() {
            ...
            while (true) {
                synchronized (mLock) {
                    ...
                    /*
                    Timer timerValidation = new Timer();
                    timerValidation.schedule(new TimerTask() {
                        @Override
                        public void run() {
                    ...
                    */
                    ...
                    //Timber.d(TAG+"PUT_RESIZE_IMAGE - "+resizedbitmap1.getWidth() +" | "+ resizedbitmap1.getHeight());
                    //Timber.d(TAG+"PUT_RESIZE_IMAGE - "+mPreviewSize.getWidth() +" | "+ mPreviewSize.getHeight());
                    //int size = resizedbitmap1.getRowBytes() * resizedbitmap1.getHeight();
                    //ByteBuffer byteBuffer = ByteBuffer.allocate(size);
                    //resizedbitmap1.copyPixelsToBuffer(byteBuffer);
                    //mPendingFrameData = byteBuffer;
                    Timber.d("START_BOX_SCAN - FrameBuild...");
                    outputFrame = new Frame.Builder()
                            //.setBitmap(resizedbitmap1)
